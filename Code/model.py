# 模型文件，通过调用make_model方法传入相关模型参数，实现对模型的初始化
...
# encoder的层数，embedding的维度，feed forward第一个全连接层维数，Dropout，multi-head的数量，设备类型
...
class Embeddings(nn.Module):
    """
    模型中，在两个embedding层和pre-softmax线性变换层之间共享相同的权重矩阵
    ...
    # 2020.12.16遇到的错误，x所含元素的数值不能大于等于vocab.
...
def attention(query, key, value, mask=None, dropout=None):
    """
    h代表每个多少个头部，这里使矩阵之间的乘法操作，对所有的head进行同时计算
...
class SublayerConnection(nn.Module):
    """
    sublayerConnection把 Multi-Head Attention和Feed Forward层连在一起
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, mask):
        # 将embedding输入进行multi_head_attention
        # 得到 attention之后的结果
        """
...
def make_model(src_vocab, tgt_vocab, N=LAYERS, d_model=D_MODEL, d_ff=D_FF, h=H_NUM, dropout=DROPOUT):
    # 输入：英文词典数，中文词典数，块的个数，embedding的维度，第一个全连接层的维度，multi-head的个数，drop率
    c = copy.deepcopy
    # 实例化Attention对象
